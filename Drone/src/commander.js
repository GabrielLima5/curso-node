class Commander{
    ...
    sendFlip(side){
        return new Promise((resolve, reject) => {
            this.socket.send(`flip ${side}`, 0, `flip ${side}`.length, this.port, this.host, (err) => {
                if (err) {
                    return reject(err)
                } else {
                    return resolve()
    ...
    sendCw(degree = 20){
        return new Promise((resolve, reject) => {
            this.socket.send(`cw ${degree}`, 0, `cw ${degree}`.length, this.port, this.host, err => {
                if (err){
                    return reject(err)
                } else {
                    return resolve()
    ...
    sendCcw(degree = 20){
        return new Promise((resolve, reject) => {
            this.socket.send(`ccw ${degree}`, 0, `ccw ${degree}`.length, this.port, this.host, err => {
                if (err) {
                    return reject(err)
                } else {
                    return resolve()
